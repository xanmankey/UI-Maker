...
class _CreatorContextMenuState extends State<CreatorContextMenu> {
  ...
  final TextEditingController descriptionController = TextEditingController();
  late Isar isarDB;
  // TODO: These have to somehow be accessible in testing so the individual
  // MenuItems can be clicked. I could create seperate widgets for them...
  static late NCM.MenuItem titleItem;
  late NCM.MenuItem descriptionItem = NCM.MenuItem(
      title: "Description",
      onSelected: () async {
        await showDialog(
          context: context,
  ...
  late NCM.MenuItem enabledItem = NCM.MenuItem(
    title: "Disable/Enable",
    items: [
      NCM.MenuItem(
        title: "Enable",
  ...
  late NCM.MenuItem colorItem = NCM.MenuItem(
    title: "Color",
    onSelected: () async {
      await showDialog(
        context: context,
  ...
  late NCM.MenuItem deleteItem = NCM.MenuItem(
    title: "Delete",
    onSelected: () async {
      await showDialog(
        context: context,
        builder: ((context) => AlertDialog(
              ...
              content: IconButton(
                onPressed: () async {
                  setState(() async {
                    await db.deleteWidgetSettings([widget.widgetSetting.id]);
                    for (WidgetSettings widget
                        in widget.widgetSetting.widgets) {
                      await db.deleteWidgetSettings([widget.id]);
                    }
  ...
  Widget build(BuildContext context) {
    return NCM.ContextMenuRegion(
      menuItems: [
        ...
        colorItem,
        deleteItem,
        // TODO: I don't understand this code; I didn't document it...
        // setState(() {
        //   widget.widgetSetting.title = descriptionController.text;
        // });
      ],
